...
Page({
  data: {
    ...
    dangqiantime:"2020-05-20",
    //控制头部提醒
    qtshow:true,
    tixing: true,
    containermarginTop: "224",
    duihuashow: true,
 ...
 //监听前贴广告
  ...
  onLoad: function () { 
    ...
    _this.setData({
      jngaodu: data.top * 2,
      jngaodu2: data.top * 2 + 80,
      containermarginTop:data.top * 2 + 80 + 100,
      jnheight: data.height * 2,
      jnwidth: data.width * 2
    })
    ...
    wx.login({
      success: function (data) {
        ...
        wx.request({
          ...
          data: {
            appid: "wxaaa30a07564110da",
            secret: "842123aeba1d7305900a63c85a11ffc6",
          ...
          success: function (res) {
            ...
            wx.getStorage({
              key: 'openid',
              success (res) {
                console.log("打印缓存"+res.data)
              },
              fail(err){
                console.log("缓存中没有openid")
                   // 上传openid到后台
                   wx.request({
                    url: 'http://www.qrbwxbz.cn:3031/openid/openidzf',
                    method: 'POST',
                    data: { 
                      openid: res.data.openid,
                    },
    ...
    //第二步  获取access_token（因为access_token是动态的，所以让后端实时去获取）

    // wx.request({
    //   url: 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wxaaa30a07564110da&secret&secret=842123aeba1d7305900a63c85a11ffc6',
    //   method: "GET",
    //   success: function (res) {
  ...
  onPullDownRefresh: function () {
    ...
    switch (page) {
      case 0:
        //  1、推荐页下拉刷新   
        wx.request({
          url: "https://www.xpmurzt.cn/api/ArticleZhufu",
          data: { lim: 6, sk: sktj },
          method: "GET",
          success: function (res) {
  ...
  // 1、 推荐页----追加-------------- 
    ...
    switch (page) {
      ...
      zhiding() {
        let that = this
        wx.request({
          url: "https://www.xpmurzt.cn/api/ArticleZhiding",
          method: "GET",
          success: function (res) {
  ...
  duihua() {
    ...
    if (_this.data.duihuashow) {
      ...
      // 点击了提醒，执行方法
      // console.log(_this.data.tongzhi.title)
      console.log("点击订阅")

      console.log(_this.data.openid)
      // console.log(_this.data.tongzhi.title)
      var dangqiantime = new Date()
           console.log(dangqiantime)
          //  _this.setData({
          //    dangqiantime:dangqiantime
          //  })
           console.log(_this.data.dangqiantime)
      wx.requestSubscribeMessage({
        tmplIds: ["APdCz_WpZe3gLwyIARBaeftYH4b5niD6eHl0_ragf-c"],
          success: function (res) {
            //成功
            console.log("发送成功")
            //用户同意了订阅，允许订阅消息
            if(res.errMsg = "requestSubscribeMessage:ok"){
              ...
              wx.request({
                url: 'http://www.qrbwxbz.cn:3031/openid/sendMessage',
                method: 'POST',
                data: { 
  ...
  tixingclose() {
    console.log("关闭顶部提示框")
    let _this = this
    console.log(_this.data.jngaodu)
    _this.setData({
      tixing: false,
      // containermarginTop: "130rpx",
      containermarginTop:  _this.data.jngaodu + 80 ,
    })
  },
  //换一批
  huanyipi() {
